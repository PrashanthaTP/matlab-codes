...
title('Filtered signal (Hamming LPF)');
xlabel('Time-->');
ylabel('Amplitude');
grid on;
...
figure(2);
freqz(h);
title({['Hamming LPF ,' , 'Order :' ,num2str(N),' (Without ','inbuilt ' 'function)'] ;['Magnitude ',' and ' ,'Phase plots']});


figure(3)
m= fir1(N,2*fc/fsamp,'low',hamming(N+1));
freqz(m);

title({['Hamming LPF ,' , 'Order :' ,num2str(N)] ;['Magnitude ',' and ' ,'Phase plots']});
...
% 
% figure(2);
% m= fir1(N-1,2*fc/fsamp,'high',hamming(N));
% freqz(m);
% title('Hamming HPF ');
% figure(3);
...
% 
% figure(4);
% m= fir1(N-1,2*fc/fsamp,'high',hann(N));
% freqz(m);
% title('Hanning HPF ');